...
const packageNames = fs
  ...
  .filter((p) => p[0] !== ".")
  .map((p) => {
    return require(path.join(rootDir, "packages", p, "package.json")).name;
  }, {});
...
const arcgisRestJsPackageNames = Object.keys(peerDependencies).filter((key) =>
  /@esri\/arcgis-rest/.test(key)
);

const hubJsPackageNames = Object.keys(peerDependencies).filter((key) =>
  /@esri\/hub-/.test(key)
);
